...
tmp = data %>% select(-r,-rate) %>% poisson_locfit_model(window=7, deg = 2)
...
tmp2 = data2 %>% select(-r,-rate) %>% group_by(class) %>% group_modify(poisson_locfit_model, window=7, deg = 1)
...
ggplot(tmp2,ggplot2::aes(x=time,y=growth.0.5, ymin=growth.0.025, ymax=growth.0.975,colour=class,groups=class))+
  ...
  ggplot2::geom_line(mapping=ggplot2::aes(x=time,y=r, colour=class), data=data2,inherit.aes = FALSE)+facet_wrap(vars(class), ncol=1)
```
...
tmp3 = data2 %>% 
  ...
  group_by(class) %>% 
  group_modify(proportion_locfit_model, window=14, deg = 2)
...
tmp6 = data2 %>%
  ...
  mutate(denom = sum(count)) %>%
  group_by(class) %>% 
  group_modify(proportion_glm_model, window=14, deg = 2)
