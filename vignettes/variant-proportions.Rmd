...
# variants2 %>% dplyr::filter(lineage_class == "Unclassified") %>% dplyr::filter(WeekEndDate > "2022-12-01") %>% tibble::view()
...
# devtools::load_all()
probs = variants_data %>% dplyr::select(-date) %>% dplyr::group_modify(multinomial_nnet_model, window = 28) %>% 
...
probs2 = variants_data %>% dplyr::select(-date) %>% 
  dplyr::group_by(place,time) %>% 
  dplyr::mutate(denom = sum(count)) %>%
  dplyr::group_by(place,class) %>%
  dplyr::group_modify(proportion_locfit_model, window = 14) %>% 
  # This is needed to map the "time period" in time back to a normal "date"
  dplyr::mutate(date = time_to_date(time))
