...
lineage_key_df = bind_rows(lineage_key_df, recombinant_key_df)
...
# variants2 %>% filter(lineage_class == "Unclassified") %>% filter(WeekEndDate > "2022-12-01") %>% view()
...
variants_data = dplyr::bind_rows(
  ...
  mutate(time = as.time_period(date)) %>% 
...
# devtools::load_all()
probs = variants_data %>% select(-date) %>% group_modify(multinomial_nnet_model, window = 28) %>% 
  # This is needed to map the "time period" in time back to a normal "date"
  mutate(date = time_to_date(time))


ggplot(probs, ggplot2::aes(x=date,y=proportion.0.5,fill=class))+ggplot2::geom_area(color="black")+
  # ggplot2::scale_x_date(date_breaks = "2 month",date_labels = "%b %y",name = NULL)+
  ggplot2::scale_fill_viridis_d()+
  ggplot2::facet_wrap(dplyr::vars(place), ncol=1)+
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle=60,vjust = 1,hjust=1))
```

```{r}

probs2 = variants_data %>% select(-date) %>% 
  group_by(place,time) %>% 
  mutate(denom = sum(count)) %>%
  group_by(place,class) %>%
  group_modify(proportion_locfit_model, window = 14) %>% 
  # This is needed to map the "time period" in time back to a normal "date"
  mutate(date = time_to_date(time))

ggplot2::ggplot(probs2, ggplot2::aes(x=date,y=proportion.0.5,ymin=proportion.0.025,ymax=proportion.0.975,fill=class,colour=class))+
  ggplot2::geom_ribbon(colour=NA,alpha=0.2)+
  ggplot2::geom_line()+
  # ggplot2::scale_x_date(date_breaks = "2 month",date_labels = "%b %y",name = NULL)+
...
ggplot2::ggplot(probs2, ggplot2::aes(x=time,y=relative.growth.0.5,ymin=relative.growth.0.025,ymax=relative.growth.0.975,fill=class,colour=class))+
  ggplot2::geom_ribbon(colour=NA,alpha=0.2)+
  ggplot2::geom_line()+
  # ggplot2::scale_x_date(date_breaks = "2 month",date_labels = "%b %y",name = NULL)+
  ggplot2::scale_fill_viridis_d(aesthetics = c("fill","colour"))+
  ggplot2::facet_wrap(dplyr::vars(place), ncol=1, scales = "free_y")+
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle=60,vjust = 1,hjust=1))+
