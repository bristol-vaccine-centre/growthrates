...
reband_discrete = function(x, y, xout, xlim=c(0,NA), ytotal=c(0,sum(y)), digits=0, labelling = c("positive_integer","inclusive","exclusive")) {
  ...
  if (labelling == "positive_integer") {
    nms = dplyr::case_when(
      !is.finite(xout) ~ sprintf("%.0f+",dplyr::lag(xout,default = 0)+1),
      dplyr::lag(xout,default = 0)+1 == xout ~ sprintf("%.0f",xout),
      is.na(dplyr::lag(xout)) ~ sprintf("0\u2014%.0f",xout),
      TRUE ~ sprintf("%.0f\u2014%.0f",dplyr::lag(xout,default = 0)+1,xout)
    )
